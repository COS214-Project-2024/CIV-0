...
TEST_CASE("UtilityFactoryTest test") {
	...
	CHECK(entity->getYPosition() == 0);
	delete entity;
	entity = utilityFactory.createEntity(EntityType::WASTEMANAGMENT, Size::SMALL, 0, 0);
	CHECK(entity != nullptr);
	CHECK(entity->getXPosition() == 0);
	CHECK(entity->getYPosition() == 0);
	delete entity;
	entity = utilityFactory.createEntity(EntityType::WASTEMANAGMENT, Size::MEDIUM, 0, 0);
	CHECK(entity != nullptr);
	CHECK(entity->getXPosition() == 0);
	CHECK(entity->getYPosition() == 0);
	delete entity;
	entity = utilityFactory.createEntity(EntityType::WASTEMANAGMENT, Size::LARGE, 0, 0);
	CHECK(entity != nullptr);
	CHECK(entity->getXPosition() == 0);
	CHECK(entity->getYPosition() == 0);
	delete entity;
