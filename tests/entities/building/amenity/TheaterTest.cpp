...
TEST_SUITE("Theater Tests") {
    TEST_CASE("Constructor Test") {
        Theater theater(ConfigManager::getEntityConfig(EntityType::THEATER, Size::SMALL), Size::SMALL, 0, 0);
        CHECK(theater.getXPosition() == 0);
        CHECK(theater.getYPosition() == 0);
        CHECK(theater.getWidth() == 5);
        CHECK(theater.getHeight() == 5);
        CHECK(theater.getRevenue() == 4000);
    ...
    TEST_CASE("isWithinEffectRadius Test") {
        ...
        SUBCASE("Theater exactly on border") {
            ...
            CHECK(baseTheater.isWithinEffectRadius(&borderTheater) == false);
        }
